...
func Register(r *gin.Engine) error {
	api := NewOpsApi()
	r.Bind(&devopsService.OpsProvider{})
...
func NewOpsApi() *OpsApi {
	service := NewService()
	return &OpsApi{service: service}
}
...
func (api *OpsApi) Demo2(c *gin.Context) {
	demoProvider := c.MustMake(devopsService.DemoKey).(devopsService.IService)
	students := demoProvider.GetAllStudent()
	usersDTO := StudentsToUserDTOs(students)
	c.JSON(200, usersDTO)
}

func (api *OpsApi) DemoPost(c *gin.Context) {
	type Foo struct {
		Name string
	}
	foo := &Foo{}
