...
import (
	"github.com/go-ddh/nice/framework/provider/log/formatter"
	"io"
	"strings"

	"github.com/go-ddh/nice/framework"
	"github.com/go-ddh/nice/framework/contract"
	"github.com/go-ddh/nice/framework/provider/log/services"
)
...
func (l *NiceLogServiceProvider) Register(c framework.Container) framework.NewInstance {
	if l.Driver == "" {
		...
		if err != nil {
			// 默认使用console
			return services.NewNiceConsoleLog
		}
	...
	switch l.Driver {
	case "single":
		return services.NewNiceSingleLog
	case "rotate":
		return services.NewNiceRotateLog
	case "console":
		return services.NewNiceConsoleLog
	case "custom":
		return services.NewNiceCustomLog
	default:
		return services.NewNiceConsoleLog
	}
}
